...
def define_mnmc_start_language(coartic_strength = 0.2):
    """
    ...
    high_nas = Category.build_params(mean_nasality=1 + coartic_strength, mean_height=5, s_nasality=.75, s_height=.75,
                                     c_nasality_height=-0)
    low_or = Category.build_params(mean_nasality=1, mean_height=0, s_nasality=.75, s_height=.75, c_nasality_height=0)
    low_nas = Category.build_params(mean_nasality=1 + coartic_strength, mean_height=0, s_nasality=.75, s_height=.75,
                                    c_nasality_height=0)
    return Language(vowels=[high_or, high_nas, low_or, low_nas], name="mnmc_start")
...
def nasal_splits(cats: list, threshold=2.5):
    ...
    #the number of those split pairs that have a high height (above threshold) or low height (below threshold)
    #and other_splits (defunct, from before binary threshold, always 0 for backwards compatibility)
    num_nasal_splits = 0
    high_splits = 0
    low_splits = 0
    other_splits = 0
    for cat_pair in itertools.combinations(cats,2):
        ...
        if nasal_split(cat_pair[0], cat_pair[1]):
            ...
            if cat_pair[0].mean[1] > threshold:
                high_splits += 1
...
def output_learners(filename:str, learners:list, seeds_to_plot: list = None, plot_dir = "", record_names = False, threshold=2.5, filetype = "pdf"):
    """
    ...
    for learner in learners:
        ...
        num_splits, high_splits, low_splits, other_splits = nasal_splits(effective_cats, threshold=threshold)
        ...
        if record_names:
            row["Name"] = learner.name
        if learner.seed in seeds_to_plot:
            filename = plot_dir + "/" + "_".join([learner.name if learner.name else "", learner.inputLanguage.name if learner.inputLanguage.name else "",
                                                  str(learner.seed), str(learner.concentration),
                                                 str(learner.numSamples), str(learner.max_iters),
                                                  str(learner.maxCats), str(learner.dpgmm.converged_)]) + "." + filetype
            title = " ".join([learner.name if learner.name else "", str(learner.seed),
...
if __name__ == "__main__":
    ...
    if cleanup:
        ...
        challenge_sphere = define_challenge_circle()
        mnmc_start = define_mnmc_start_language()

        #Plot languages
        seed = 9
        numSamples = 250
        language_output_dir = "simulated_input_distributions"
        ideal_models_csv = open(language_output_dir+"/"+"ideal_aris.csv", "w+")
        ideal_writer = csv.DictWriter(ideal_models_csv, fieldnames=["Language", "Seed","NumSamples","IdealARI"])
        for language in [easy_language, overlap_language, challenge_language, scikit_lang, challenge_lang_2, challenge_sphere, mnmc_start]:
        ...
        if debug:
            ...
            for num_samples in [250, 300, 350, 400, 450, 500]:
                run_concentration_trials(mnmc_start, seeds_to_view=[0,1,17,18,9], numSamples=num_samples, maxIters=3000)
